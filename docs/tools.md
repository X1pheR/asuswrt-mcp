...
| `asuswrt_remote_syslog` | Management | Enable forwarding to one private RFC1918 IPv4 address and UDP port, or disable forwarding. Changes only remote destination/port, restarts the logger and verifies NVRAM readback; local logging settings are preserved. Requires mutation enablement and confirmation; dry-run is non-mutating. Destination values are omitted from results. |
